 ...
 *   • a LOCATION page  — `/care-staffing-agency-in-camden` (page_type=borough)
 *   • an ARTICLE       — `/care-home-cpd`                   (page_type=blog)
 * and for each asserts: one banner/main/contentinfo landmark + single h1; a non-skipping
 * heading outline (no level jumps by >1); and zero serious/critical Axe violations.
...
const PAGES: Array<{ label: string; path: string }> = [
  { label: 'location page (borough)', path: '/care-staffing-agency-in-camden' },
  { label: 'article (blog)', path: '/care-home-cpd' },
];
