...
// phone/email/address/trading_name are never literals, so the cleaning/security
// number can never appear on Care.
export function Footer({ page }: { page: PageJson }) {
  const b = page.brand;
      ...
      <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-3">
        ...
        <div>
          <a href={page.nap.enquiry_url} style={{ background: b.cta, color: b.bg }}
             className="inline-block rounded-lg px-5 py-3 text-sm font-medium">Request care support</a>
          <p className="mt-6 text-[12px] opacity-40">{siteNav.companyReg}</p>
        </div>
      </div>
    </footer>
  );
